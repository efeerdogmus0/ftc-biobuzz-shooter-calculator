...
The Coulomb cap supports sliding and the effective-mass clamp supports sticking. Unknown wheel inertia is an ideal-speed boundary: inverse inertia is zero and no measured inertia-dependent metric is claimed. A passive hood has zero surface speed. Each powered Sushi wheel has an explicit local center, radius, angular placement, compression offset and contact interval. Its mount angle projects rim speed and torque onto the local launch path. At every integration step its penetration is `Rb + Ri - distance(ballCenter, rollerCenter)` after the configured compression offsets; only rollers with positive penetration apply an impulse. This permits changing #1, #1+#2 and #2+#3 contact combinations along the path. The default three 1 in Sushi wheels are gear-linked at the editable 3.75 hood/main RPM ratio. Contact terminates at traveled hood arc length or the configurable contact time limit; an incomplete transit is warned.
...
The scoring opening is the active blue CELL: a local oriented pentagon with a 20 in width, 14 in total height, 7.61 in shoulders and 12.04 in depth. Whole-ball entry applies convex-polygon erosion: every edge is evaluated by its inward signed distance and must exceed projectile radius. This gives the sloped roof its normal-direction clearance. Side walls, back/bottom, the HIVE footprint/crossbar AABBs and perimeter walls are checked continuously. Coverage also rejects robot footprints that overlap a HIVE obstacle. HIT is entry before contact; the ball then stops at its first solid collision. No restitution, bounce or retention is claimed. If no opening-plane crossing exists, miss distance is closest sampled distance to center, rather than opening clearance.
